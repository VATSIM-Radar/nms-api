...
export async function getAirportNotams(icao: string): Promise<VatsimAirportDataNotam[]> {
    ...
    if (!token || expire < Date.now()) {
        ...
        token = faaTokenRequest.access_token;
        expire = Date.now() + (1000 * (Number(faaTokenRequest.expires_in) - 5));
    }
